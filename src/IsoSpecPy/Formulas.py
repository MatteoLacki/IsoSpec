...
def averagine(target_avg_mass):
    return "".join(
        ...
        for key, val in sorted(
            averagine_dct(target_avg_mass).items(), key=lambda x: x[0]
        )
    )


acgt_avg_base = Counter({"C": 9.75, "H": 12.25, "N": 3.75, "O": 6.0, "P": 1.0})
...
def acgt(no_bases, add_water=True):
    ...
    return "".join(
        str(key) + str(val) for key, val in sorted(c.items(), key=lambda x: x[0])
    )
