                     ...
                     bool get_lprobs, bool get_confs  )
{
    ...
    _masses = get_masses ? malloc(INIT_TABLE_SIZE * sizeof(double)) : nullptr;
    _lprobs = get_lprobs ? malloc(INIT_TABLE_SIZE * sizeof(double)) : nullptr;
    _probs  = get_probs  ? malloc(INIT_TABLE_SIZE * sizeof(double)) : nullptr;
    _confs  = get_confs  ? malloc(INIT_TABLE_SIZE * allDimSizeOfInt): nullptr;
    ...
    while(advanceToNextConfigurationIsoThresholdGenerator(generator)){
        ...
        if( _confs  != nullptr ){
            memcpy(_confs + confs_tbl_idx, get_conf_signature());
            confs_tbl_idx += generator->getAllDim();
        }
    ...
    _masses = realloc(_masses, confs_no * sizeof(double));
    _lprobs = realloc(_lprobs, confs_no * sizeof(double));
    _probs  = realloc(_probs,  confs_no * sizeof(double));
    _confs  = realloc(_confs,  confs_tbl_idx * sizeof(int));
}
