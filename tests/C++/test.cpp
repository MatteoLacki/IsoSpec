...
int main()
{
    ...
    iso = IsoSpec::IsoFromFormula("C100O100", 10.0);
    iso->processConfigurationsUntilCutoff();
    product = iso->getCurrentProduct();
    delete[] std::get<0>(product);
    delete[] std::get<1>(product);
    delete[] std::get<2>(product);

    delete iso;


    iso = IsoSpec::IsoFromFormula("C1000O1000N1000H1000S1000", 0.99);
    iso->processConfigurationsUntilCutoff();
    product = iso->getCurrentProduct();
    delete[] std::get<0>(product);
    delete[] std::get<1>(product);
